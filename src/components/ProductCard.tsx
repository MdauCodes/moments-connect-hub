      ...
      <div className="relative flex items-start gap-6 px-5">
        ...
        <div className="min-w-0 flex-1">
          <p className="font-mono text-[10px] uppercase tracking-[0.18em] text-muted-foreground">
            {product.division === "food" ? "Food Service" : "Retail & Industrial"}
          </p>
          <h3 className="mt-1.5 font-display text-2xl font-medium leading-tight text-foreground">
            {product.name}
          </h3>
          <div className="mt-3 flex flex-wrap items-baseline gap-x-6 gap-y-1 font-mono text-[11px] uppercase tracking-widest text-muted-foreground">
            <span>MOQ <span className="text-foreground">{product.moq.toLocaleString()}</span></span>
            <span>Sizes <span className="text-foreground">{product.sizes.length}</span></span>
            <span>Lead <span className="text-foreground">7–14 days</span></span>
          </div>
        </div>
