...
function AdminProductsPage() {
  ...
  return <AdminLayout title="Products" actionLabel="New product" onAction={beginCreate}><div className="admin-page-stack">
    ...
    <div className="admin-panel" data-admin-table-scroll><table className="admin-table"><thead><tr><th>Product</th><th>Category</th><th>Flags</th><th>MOQ</th><th></th></tr></thead><tbody>{loading ? <tr><td colSpan={5}>Loading products…</td></tr> : products.length === 0 ? <tr><td colSpan={5}><div className="admin-empty">No products found. <button className="admin-btn admin-btn-primary" onClick={beginCreate}>Create product</button></div></td></tr> : products.map((p) => <tr key={p.id}><td><div style={{ display: "flex", gap: 10, alignItems: "center" }}>{(p.primaryImageUrl || p.imageUrls?.[0]) && <img src={p.primaryImageUrl || p.imageUrls?.[0]} alt="" style={{ width: 44, height: 38, objectFit: "cover", borderRadius: 6 }} />}<b>{p.name}</b></div></td><td>{p.category || "—"}</td><td>{[p.isDiscount && "Discount", p.isNewArrival && "New", p.isFastMoving && "Fast"].filter(Boolean).join(" · ") || "—"}</td><td>{p.moq ?? 1}</td><td><button className="admin-btn admin-btn-ghost" onClick={() => beginEdit(p)}><Pencil size={14} />Edit</button>{isAdmin && <button className="admin-btn admin-btn-danger" onClick={() => void remove(p)}><Trash2 size={14} />Delete</button>}</td></tr>)}</tbody></table></div>
